                        ...
                        @if (isset($user))
                            <tr class="gradeX even">
                                ...
                                @if (intval($combPeriod) >= intval($user->archievements->min('period')))
                                    <td>{{ number_format($user->archievements->whereBetween('period', [$user->archievements->min('period'), $combPeriod])->sum('total_pv') ??floatval(0),2) }}
                                    </td>
                                @else
                                ...
                                <td
                                    class="{{ $user->currentsalary($combPeriod) && $user->currentsalary($combPeriod)->active ? '' : 'tred' }}">
                                    {{ number_format($user->currentsalary($combPeriod)->amount ?? 0, 2) }}</td>
                            </tr>
                            @foreach ($sponsers as $key => $sponser)
                                <tr class="gradeX {{ ($key + 1) % 2 == 0 ? 'even' : 'odd' }}">
                                    ...
                                    @if (intval($combPeriod) >= intval($sponser->archievements->min('period')))
                                        <td>{{ number_format($sponser->archievements->whereBetween('period', [$sponser->archievements->min('period'), $combPeriod])->sum('total_pv') ?? floatval(0),2) }}
                                        </td>
                                    @else
                                        <td>{{ number_format(floatval(0), 2) }}</td>
                                    @endif
                                    ...
                                    <td
                                        class="{{ $sponser->currentsalary($combPeriod) && $sponser->currentsalary($combPeriod)->active ? '' : 'tred' }}">
                                        {{ number_format($sponser->currentsalary($combPeriod)->amount ?? 0, 2) }}</td>
                                </tr>
                            @endforeach
                        @endif
